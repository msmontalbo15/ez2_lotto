...
class EZ2Repository {
  ...
  Future<void> loadAll({int monthCount = 3}) async {
    // 0. Deduplicate — prevents parallel calls from triggering redundant cache/network checks
    await _once('loadAll:$monthCount', () async {
      ...
      if (!anyStale) {
        Log.d('EZ2Repository', 'loadAll: all cache fresh, skipping network');
        return;
      }

      // 4. Network request
      try {
        Log.d('EZ2Repository', 'loadAll: fetching from network');
        ...
        Log.e('EZ2Repository', 'loadAll error: $e');
  ...
  Future<void> loadToday({bool force = false}) async {
    ...
    if (!force && !CacheService.isStale(cacheKey, ttl)) {
      Log.d('EZ2Repository', 'today: cache fresh, skipping network');
      return;
    }

    await _once('today:$iso', () async {
      try {
        Log.d('EZ2Repository', 'today: fetching from network');
        ...
        Log.e('EZ2Repository', 'today error: $e');
        if (cached == null) {
          _errorCtrl.add('offline');
          _todayCtrl.add(DayResult.empty(toShortDate(ph), toDayName(ph)));
        }
  ...
  void subscribeRealtime() {
    ...
    _channel = ApiService.subscribeToDate(
      isoDate: iso,
      onChanged: () async {
        Log.d('EZ2Repository', 'realtime: change detected for $iso');
  ...
  DayResult? _cachedToday(String iso, DateTime ph) {
    ...
    try {
      ...
      Log.e('EZ2Repository', 'today cache parse error: $e');
      return null;
    }
  }
  ...
  List<DayResult>? _cachedMonth(String mk) {
    ...
    try {
      ...
      Log.e('EZ2Repository', 'month cache parse error: $e');
      return null;
    }
  }
  ...
  void _scrapeBackground(String iso, DateTime ph) {
    ...
    // Throttle: don't scrape more than once every 2 minutes
    if (_lastScrapeAttempt != null && now.difference(_lastScrapeAttempt!) < const Duration(minutes: 2)) {
      return;
    }
    _lastScrapeAttempt = now;

    ApiService.triggerScrapeToday().then((slots) async {
      if (slots.isNotEmpty) {
        Log.d('EZ2Repository', 'scrape returned: $slots');
        await CacheService.delete(CacheService.todayKey(iso));
        await loadToday(force: true);
      }
    }).catchError((Object e, StackTrace st) async {
      Log.e('EZ2Repository', 'scrape error: $e');
    }, test: (e) => true);
  }
}
