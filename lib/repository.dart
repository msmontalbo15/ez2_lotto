...
class EZ2Repository {
  ...
  Future<void> loadToday({bool forceNetwork = false}) async {
    ...
    if (cached != null) {
      ...
      // Cache is fresh and not in draw window — skip network
      if (!forceNetwork &&
    ...
    // 2. Network fetch — AWAITED so callers (Future.wait) know when done
    await _fetchTodayFromNetwork(iso, ph, cacheKey, cached == null);
  }
  ...
  Future<void> _fetchTodayFromNetwork(
    ...
    try {
      ...
      if (_shouldScrape(result, ph)) {
        _scrapeInBackground(iso, ph, cacheKey);
      }
    } catch (e) {
  ...
  void _scrapeInBackground(String iso, DateTime ph, String cacheKey) async {
    try {
      ...
      if (newSlots.isNotEmpty) {
        final fresh = await _db
            ...
            .order('draw_slot')
            .timeout(const Duration(seconds: 10));
        final list = List<Map<String, dynamic>>.from(fresh as List);
        final result = _buildToday(list, ph);
        await CacheService.set(cacheKey, list);
        _todayCtrl.add(result);
      }
  ...
  Future<void> loadAll({int monthCount = 3}) async {
    ...
    // Emit cache for all months immediately
    ...
    // One single network request for everything
    try {
