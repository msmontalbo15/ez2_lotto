...
String _monthLabel(String mk) {
  ...
  return months[m];   // year shown separately in year row
}
...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Pre-fetch the 3 most recent months so they're ready
...
<<<<<<< HEAD
    ...
    final isLoading  = prov.historyLoading[mk] == true && (rows == null || rows.isEmpty);
...
>>>>>>> 040d0d8d8116ae221e5f6e6341a7441b44ce6370
    ...
    if (rows != null) {
      ...
      for (final r in rows) rowMap[r.date] = r;
      ...
      if (isCurrent) {
        ...
        if (liveToday != null && liveToday.hasAnyResult) {
          ...
          rowMap[todayDate] = DayResult(date: todayDate, day: toDayName(ph));
        }
      }
...
>>>>>>> 040d0d8d8116ae221e5f6e6341a7441b44ce6370
              ...
              // ── Year selector ──────────────────────────
              _YearSelector(
                years:        prov.availableYears,
                selectedYear: prov.selectedYear,
                onSelect:     (y) => prov.setYear(y),
              ),
...
<<<<<<< HEAD
            child: RefreshIndicator(
              onRefresh: () async {
                final prov = context.read<AppProvider>();
                await prov.fetchMonth(prov.selectedMonth);
              },
              color: const Color(0xFF1A5276),
              child: isLoading
                  ? _SkeletonList(hPad: context.horizontalPadding)
                  : visible.isEmpty
                      ? _EmptyState(onRetry: () => prov.fetchMonth(mk))
                      : ListView.builder(
                          physics: const AlwaysScrollableScrollPhysics(),
                          padding: EdgeInsets.fromLTRB(
                              context.horizontalPadding, 8,
            ...
            child: AnimatedContainer(
              ...
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 14,
                  fontWeight: FontWeight.w800,
                  color: sel ? const Color(0xFF1A5276) : Colors.white,
                ),
              ),
            ),
          );
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: months.map((m) {
          ...
          return GestureDetector(
            behavior: HitTestBehavior.opaque,  // ← fixes tap not registering
            onTap: () => onSelect(m),
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 180),
