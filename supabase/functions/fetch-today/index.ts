...
const REGEX = {
  ...
  row:
    /<tr[^>]*>[\s\S]*?(\d{1,2}:\d{2}\s*(?:AM|PM))[\s\S]*?<\/td>[\s\S]*?<td[^>]*>([\d]{1,2}[\s\-–][\d]{1,2})<\/td>/gi,
  ...
  winnersB:
    /(\d{1,2}:\d{2}\s*(?:AM|PM))[\s\S]{0,500}?winners?\s*:?\s*(\d[\d,]+)/gi,

  // Match winners in table column: <td>2:00 PM</td><td>01-02</td><td>123</td>
  winnersC:
    /<td[^>]*>\s*(\d{1,2}:\d{2}\s*(?:AM|PM))\s*<\/td>\s*<td[^>]*>[\d\-]+<\/td>\s*<td[^>]*>\s*(\d[\d,]*)\s*<\/td>/gi,

  // Match pcsoball row
  pcsoball:
    /<span class="lotto-ball"[^>]*>\s*(\d+)\s*<\/span>\s*<span class="lotto-ball"[^>]*>\s*(\d+)\s*<\/span>[\s\S]*?<a href="[^"]*2DLotto(2PM|5PM|9PM)\/([^"]+)"[^>]*>\s*([^<]+)\s*<\/a>[\s\S]*?<span class="result-winners-badge[^"]*">\s*([\d,]+)\s*winner/gi,
} as const;
...
function normalizeCombo(raw: string): string | null {
  ...
  if (isNaN(n1) || isNaN(n2) || n1 < 1 || n1 > 31 || n2 < 1 || n2 > 31) {
    return null;
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const iso = `${ph.getUTCFullYear()}-${
      String(ph.getUTCMonth() + 1).padStart(2, "0")
    }-${String(ph.getUTCDate()).padStart(2, "0")}`;
    ...
    const db = new Map<
      string,
      { combo: string | null; winners: number | null }
    >();
    (existing ?? []).forEach((r) => {
    ...
    const needsWinners = slots.filter((s) =>
      db.get(s)?.combo && db.get(s)?.winners == null
    );
    ...
    const urls = [
      ...
      `https://pcsoball.com/result/game/2DLotto`,
    ];
    ...
    const fetchWithBackoff = async (
      ...
      for (let attempt = 0; attempt < retries; attempt++) {
        try {
          ...
          console.log(
            `[fetch-today] Attempt ${attempt + 1} failed for ${url}:`,
    ...
    for (const html of results) {
      ...
      // Parse pcsoball
      REGEX.pcsoball.lastIndex = 0;
      while ((match = REGEX.pcsoball.exec(html)) !== null) {
        const dateStr = match[4]; // e.g. "2026-05-13"
        if (dateStr !== iso) continue; // Only process today's date

        const slot = match[3].toLowerCase(); // "2pm", "5pm", "9pm"
        const num1 = match[1];
        const num2 = match[2];
        const combo = `${num1.padStart(2, "0")}-${num2.padStart(2, "0")}`;
        const winners = parseInt(match[6].replace(/,/g, ""), 10);

        if (!scraped[slot]) {
          scraped[slot] = { combo, winners };
        } else {
          if (!scraped[slot].combo) scraped[slot].combo = combo;
          if (scraped[slot].winners == null && !isNaN(winners)) {
            scraped[slot].winners = winners;
          }
        }
      }
    ...
    console.log(
      `[fetch-today] ${iso}: inWindow=${inWindow}, needsCombo=${needsCombo}, needsWinners=${needsWinners}`,
    );
    ...
    for (const slot of slots) {
      ...
      if (!dbRow?.combo) {
        ...
        if (!error) {
          saved.push(slot);
          console.log(
            `[fetch-today] Inserted ${slot}: ${s.combo}, winners=${winnersFinal}`,
          );
        } else {
      ...
      if (dbRow.combo !== s.combo) {
        ...
        if (!error) {
          ...
          console.log(
            `[fetch-today] Corrected ${slot}: ${dbRow.combo} → ${s.combo}`,
          );
        }
        continue;
      }
    ...
    await supabase.from("ez2_fetch_log").upsert({
      ...
      notes: saved.length > 0
        ? `Saved: ${saved.join(", ")}`
        : `Nothing new. scraped=${JSON.stringify(Object.keys(scraped))}`,
      fetched_at: new Date().toISOString(),
